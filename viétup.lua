...
QuestNeta = function()
    ...
    Title = "Abacaxi Hub",
    SubTitle = "by humano and orchidx",
...
local Setting = Library:MakeTab({
    Title = "Cài Đặt Và Khác",
    Icon = "rbxassetid://7734053495"
})
Shop:AddSection("Cửa Hàng Võ")
Shop:AddButton({
    Name = "Chân Đen",
    Callback = function()
...
function UpdateTime()
    ...
    Second = GameTime % 60
    Timmessss:SetDesc(Hour .. " Giờ (h) " ..Minute .. " Phút (m) " ..Second .. " Giây (s) ")
end
spawn(function()
    while wait() do
        UpdateTime()
...
Farm:AddSection({"Nhặt"})
-- Botão Auto Collect Chest
Farm:AddToggle({
    Name = "Tự Đọng Nhặt Rương",
    Description = "",
...
if World3 then
Farm:AddSection({"Xương"})
-- AUTO RANDOM BONES
Farm:AddToggle({
    Name = "Tự Đọng Random Xương",
    Default = false,
    Callback = function(v)
...
Others:AddToggle({
    Name = "Tự Động Nhập Nhiệm Vụ Câu",
...
Others:AddToggle({
    Name = "Tự Động Hoàn Thành Nhiệm Vụ",
...
-- 8. AUTO SPAM SKILL Z
Others:AddToggle({
    Name = "Tự động Sử Dụng Kĩ Năng Cần Câu",
...
Others:AddToggle({
    Name = "Tự động Farm Boss Đã Chọn",
    Default = false,
    Callback = function(v)
        _G.AutoBoss = v
 ...
 Others:AddToggle({
    Name = "Nhận Nhiệm Vụ Boss Đã Chọn",
    Default = false,
    Callback = function(v)
...
Others:AddSection({"Nhiệm Vụ Và Haki Quan Sát V2"})
 Others:AddToggle({
    Name = "Farm Điểm Haki Quan Sát",
    Description = "",
    -- 1. Carrega o estado salvo ou false por padrão
...
if World3 then
Others:AddToggle({
	Name = "Tự Động Lấy Haki Quan Sát V2",
	Description = "",
	-- 1. Carrega se estava ligado ou desligado ao abrir o script
...
Others:AddSection({"Yama Và Tushita"});
local r = Others:AddParagraph({ Title = "Tiến Trình Đánh Elite ", Content = "" });
spawn(function()
	while wait(Sec) do
		pcall(function()
			r:SetDesc("Tiến Độ :  " .. replicated.Remotes.CommF_:InvokeServer("EliteHunter", "Progress"));
		end);
	end;
end);
Others:AddToggle({
	Name = "Tự Động Đánh Elite",
	Description = "",
	-- 1. Carrega o estado salvo ou false por padrão
...
spawn(function()
 ...
 Others:AddToggle({
    Name = "Dừng Lại Khi Có Chén Thánh",
    Description = "",
...
Others:AddSection({"Màu Haki/Buso"});

 Others:AddToggle({
    Name = "Dịch Chuyển Đến Barista Haki",
    Description = "",
    -- 1. Carrega se estava ligado ou desligado ao abrir o script
...
spawn(function()
 ...
 Others:AddToggle({
    Name = "Nhận Nhiệm Vụ Từ Xa [Nguy Hiểm]",
    Description = "Không Nên Sử Dụng",
...
Event:AddSection({"Sự Kiện Biển & Cài Đặt"})
local z5 = {
        "Guardian",
        "PirateGrandBrigade",
        "MarineGrandBrigade",
...
Event:AddSection({"Chọn Quái Bạn Muốn Farm"})
    Event:AddToggle({
...
Event:AddToggle({
    Name = "Tự Động Đánh Pirate GrandBrigade",
    Description = "",
    Default = false,
    Callback = function(I)
        _G.PGB = I;
...
if World2 then
  Event:AddSection({"Đến Sea 3 Để Dùng Thêm"})
end
if World1 then
  Event:AddSection({"Lên Sea 3 Hoặc 2 Để Dùng"})
end
...
Event:AddToggle({
    Name = "Dịch Chuyển Đến Nơi Đánh Leviathan",
...
Event:AddSection({"Đảo Kitsune & Event"});
Event:AddToggle({
...
Event:AddToggle({
	Name = "Tự động Dịch Chuyển Đến Shrine Actived",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.tweenShrine = I;
...
Event:AddButton({ Name = "Đổi Đồ Azure", Description = "", Callback = function()
		(replicated.Modules.Net:FindFirstChild("RF/KitsuneStatuePray")):InvokeServer();
	end });
Event:AddButton({ Name = "Nói Chuyện Với kitsune statue", Description = "", Callback = function()
		(replicated.Modules.Net:FindFirstChild("RE/TouchKitsuneStatue")):FireServer();
	end });
	Event:AddSection({"Đảo Bí Ẩn Và Trăng Tròn"});
Event:AddToggle({
...
Event:AddToggle({
	Name = "Bay Đến Nơi Cao Nhất Của Đảo Bí Ẩn",
...
Event:AddToggle({
	Name = "Bay Đến Người Bán Trái Ác Quỷ Trên Đảo Bí Ẩn",
...
Event:AddSection({"Đảo Dung Nham"});
Event:AddToggle({
    Name = "Tự Động Tìm Đảo Dung Nham",
    Description = "",
...
Event:AddToggle({
    Name = "Tự Động Chơi Sự KIện Đảo Dung Nham",
    Description = "",
    -- 1. Carrega o estado salvo
...
-- 3. TOGGLE: Auto Reset
Event:AddToggle({
    Name = "Reset Khi Hoàn Thành Complete Volcano",
    Description = "",
...
if World2 or World3 then
Event:AddSection({"Sử Dụng Kỹ Năng (Võ)"})
-- [[ TOGGLES MELEE ]]
Event:AddToggle({
    Name = "Skill Z",
...
Maestry:AddToggle({
    Name = "Tự Động Farm Mastery Trái",
    Description = "",
...
Maestry:AddToggle({
    Name = "Tự Động Farm Mastery Súng",
    Description = "",
    Default = false,
    Callback = function(I)
        _G.FarmMastery_G = I
...
if World2 then
Race:AddSection({"Nâng Tộc Thường"});
Race:AddToggle({
    Name = "Tự Động Nâng Tộc Mink V2/V3",
    Description = "",
    Default = false,
    Callback = function(I)
        G.Auto_Mink = I;
...
Race:AddToggle({
    Name = "Tự Động Nâng Tộc Human V2/V3",
    Description = "",
    Default = false,
    Callback = function(I)
        G.Auto_Human = I;
...
Race:AddToggle({
    Name = "Tự Động Nâng Tộc Angel V2/V3",
    Description = "",
    Default = false,
    Callback = function(I)
        G.Auto_Skypiea = I;
...
Race:AddToggle({
    Name = "Tự Động Nâng Tộc Shark V2/V3",
    Description = "",
    Default = false,
    Callback = function(I)
        G.Auto_Fish = I;
...
Race:AddSection({"Nâng Tộc V4 Chỉ Có Ở Sea 3"});
end
if World1 then
...
NotificacaoNightMystic("Abacaxi Hub", "✅ Todos os sistemas carregados!")
